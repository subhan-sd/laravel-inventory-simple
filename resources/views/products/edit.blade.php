...
@section('title', 'Edit Product - ElectroTech')
...
<div class="max-w-2xl mx-auto" x-data="{ showUpdateModal: false, showDeleteModal: false }">
    <div class="mb-8">
        ...
        <p class="text-slate-500">Update the details for "{{ $product->name }}".</p>
    </div>

    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden p-6 md:p-8">
        <form id="updateForm" action="{{ route('products.update', $product) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" class="w-full px-4 py-2 bg-slate-50 border @error('name') border-red-300 @else border-slate-200 @enderror text-slate-900 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all" required>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" name="category" id="category" value="{{ old('category', $product->category) }}" class="w-full px-4 py-2 bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all">
                </div>

                <!-- Price -->
                <!-- <div>
                    <label for="price" class="block text-sm font-medium text-slate-700 mb-1">Price (Rp)</label>
                    <input type="number" step="0.01" name="price" id="price" value="{{ old('price', $product->price) }}" class="w-full px-4 py-2 bg-slate-50 border @error('price') border-red-300 @else border-slate-200 @enderror text-slate-900 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all" required>
                    @error('price')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div> -->
                ...
                <!-- Stock -->
                <!-- <div>
                    <label for="stock" class="block text-sm font-medium text-slate-700 mb-1">Stock Level</label>
                    <input type="number" name="stock" id="stock" value="{{ old('stock', $product->stock) }}" class="w-full px-4 py-2 bg-slate-50 border @error('stock') border-red-300 @else border-slate-200 @enderror text-slate-900 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all" required>
                    @error('stock')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div> -->
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4" class="w-full px-4 py-2 bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all">{{ old('description', $product->description) }}</textarea>
                </div>
            </div>
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-slate-500">Are you sure you want to update this product? Changes will be saved immediately.</p>
                            </div>
                        </div>
                    </div>
                </div>
